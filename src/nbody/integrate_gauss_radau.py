...
from nbody.ode_n_body_second_order import ode_n_body_second_order
import numpy as np


class Gauss_Radau(Integrator):
    def __init__(self, particles=None, CONST_G=1.0, dt=0.01):
        super(Gauss_Radau, self).__init__(particles=particles, CONST_G=CONST_G, dt=dt)
        self.tol = 1.0e-9
        self.__initialised = False

    def integrate(self, end_time, exponent=1.0/7):
        """
        For fixed timestep integration set exponent=0.0
        """
        ...
        self.sol_state = np.zeros((nstore, len(x)))
        self.sol_state[0, :] = x
        self.sol_time = np.linspace(
            self.t_start, self.t_start + self.dt * (nstore - 1), nstore
        )
        ...
        dt, ddy0 = self.__initial_time_step(y0, dy0, self.CONST_G, self.particles.masses, self.particles.N)
        ...
        self.sol_time = sol_time[0:istep]
        self.sol_state = np.concatenate((sol_pos[0:istep, :], sol_vel[0:istep, :]), axis=1)
    ...
    def gauss_radau15_step(self, y0, dy0, ddy0, ddys, dt, t, tf,
                           nh, hs, bs0, bs, E, g, r, c, tol, exponent,
                           fac, imode, G, masses, nbodies):
        istat = 0
        ...
        while True:
            ...
            for ipc in range(0, 12):
                ...
                for ih in range(0, nh):
                    ...
                    ddys[ih, :] = ode_n_body_second_order(y, self.CONST_G, self.particles.masses)
            ...
            ddy = ode_n_body_second_order(y, self.CONST_G, self.particles.masses)
                ...
                F1 + h * (
                    b[0, :] / 0.3e1 + h * (
                        b[1, :] / 0.6e1 + h * (
                            b[2, :] / 0.10e2 + h * (
                                b[3, :] / 0.15e2 + h * (
                                    b[4, :] / 0.21e2 + h * (b[5, :] / 0.28e2 + h * b[6, :] / 0.36e2)
                                )
                            )
                        )
                    )
            ...
            * (
                b[0, :] / 0.2e1 + h * (
                    b[1, :] / 0.3e1 + h * (
                        b[2, :] / 0.4e1 + h * (
                            b[3, :] / 0.5e1 + h * (
                                b[4, :] / 0.6e1 + h * (b[5, :] / 0.7e1 + h * b[6, :] / 0.8e1)
                            )
                        )
                    )
                )
    ...
    def __compute_gs(g, r, ddys, ih):
        ...
        if ih == 1:
            ...
            g[2, :] = (((F4 - F1) * r[3, 0] - g[0, :]) * r[3, 1] - g[1, :]) * r[3, 2]
            g[3, :] = (
                (((F5 - F1) * r[4, 0] - g[0, :]) * r[4, 1] - g[1, :]) * r[4, 2] - g[2, :]) * r[4, 3]
            g[4, :] = (
                ((((F6 - F1) * r[5, 0] - g[0, :]) * r[5, 1] - g[1, :]) * r[5, 2] - g[2, :]) * r[5, 3] - g[3, :]) * r[5, 4]
    ...
    def __initial_time_step(y0, dy0, G, masses, nbodies):
        ...
        # Evaluate function
        f0 = ode_n_body_second_order(y0, G, masses)
        d0 = max(abs(y0))
        d1 = max(abs(f0))
        ...
        f1 = ode_n_body_second_order(y1, G, masses)
